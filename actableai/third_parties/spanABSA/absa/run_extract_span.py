...
from actableai.third_parties.spanABSA.squad.squad_evaluate import (
    exact_match_score,
...
def read_train_data(args, tokenizer, logger):
    ...
    all_start_positions = torch.tensor(
        [f.start_positions for f in train_features], dtype=torch.long
    )
    all_end_positions = torch.tensor(
        [f.end_positions for f in train_features], dtype=torch.long
    )
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if (
                ...
                f = open(log_path, "a")
                print(
                    "step: {}, loss: {:.4f}, P: {:.4f}, R: {:.4f}, F1: {:.4f} (common: {}, retrieved: {}, relevant: {})".format(
                        global_step,
                        running_loss / count,
                        metrics["p"],
                        metrics["r"],
...
def eval_aspect_extract(
    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        pred_terms = []
        for start_index, end_index, span_mask in zip(
            result.start_indexes, result.end_indexes, result.span_masks
        ):
            if span_mask:
        ...
        for pred_term in pred_terms:
            common += metric_max_over_ground_truths(
                exact_match_score, pred_term, example.term_texts
            )
...
def evaluate(
    ...
    metrics, all_nbest_json = eval_aspect_extract(
        eval_examples,
        eval_features,
        all_results,
        args.do_lower_case,
...
def main():
    ...
    parser.add_argument(
        "--filter_type", default="f1", type=str, help="Which filter type to use"
    )
    parser.add_argument(
        "--use_heuristics",
        default=True,
    ...
    if args.do_predict:
        ...
        f = open(log_path, "a")
        print(
            "threshold: {}, step: {}, P: {:.4f}, R: {:.4f}, F1: {:.4f} (common: {}, retrieved: {}, relevant: {})".format(
                args.logit_threshold,
                global_step,
                metrics["p"],
                metrics["r"],
