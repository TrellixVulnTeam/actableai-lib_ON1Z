...
from actableai.third_parties.spanABSA.squad.squad_utils import (
    get_final_text,
    _get_best_indexes,
)
...
def convert_examples_to_features(
    examples, tokenizer, max_seq_length, verbose_logging=False, logger=None
):
    max_term_num = max(
        [len(example.term_texts) for (example_index, example) in enumerate(examples)]
    )
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for start_position, end_position in zip(
            example.start_positions, example.end_positions
        ):
        ...
        for tok_start_position, tok_end_position, polarity in zip(
            tok_start_positions, tok_end_positions, example.polarities
        ):
            if tok_start_position >= 0 and tok_end_position <= (max_seq_length - 1):
                ...
                max_term_length = (
                    term_length if term_length > max_term_length else max_term_length
                )
...
RawSpanResult = collections.namedtuple(
    "RawSpanResult", ["unique_id", "start_logits", "end_logits"]
)

RawSpanCollapsedResult = collections.namedtuple(
    "RawSpanCollapsedResult",
    [
        "unique_id",
        "neu_start_logits",
        "neu_end_logits",
...
RawBIOClsResult = collections.namedtuple(
    "RawBIOClsResult",
    ["unique_id", "start_indexes", "end_indexes", "bio_pred", "span_masks"],
)

RawFinalResult = collections.namedtuple(
    "RawFinalResult",
    ["unique_id", "start_indexes", "end_indexes", "cls_pred", "span_masks"],
)


def wrapped_get_final_text(
    example, feature, start_index, end_index, do_lower_case, verbose_logging, logger
):
    ...
    final_text = get_final_text(
        tok_text, orig_text, do_lower_case, verbose_logging, logger
    )
    return final_text


def span_annotate_candidates(
    all_examples,
    batch_features,
    batch_results,
    filter_type,
    ...
    for (feature_index, feature) in enumerate(batch_features):
        ...
        if is_training:
            ...
            for start_index, end_index, polarity_label, mask in zip(
                feature.start_indexes,
                feature.end_indexes,
                feature.polarity_labels,
                feature.label_masks,
            ):
                if (
                    mask
                    and start_index in feature.token_to_orig_map
                    and end_index in feature.token_to_orig_map
                ):
            ...
            for i, pred_i in enumerate(prelim_predictions_per_feature):
                ...
                if (i + 1) < len(prelim_predictions_per_feature) and use_nms:
                    indexes = []
                    for j, pred_j in enumerate(
                        prelim_predictions_per_feature[(i + 1) :]
                    ):
                        filter_text = wrapped_get_final_text(
                            example,
                            feature,
                            pred_j.start_index,
                            pred_j.end_index,
                        ...
                        if filter_type == "em":
                            ...
                            raise Exception
                    [
                        prelim_predictions_per_feature.pop(index - k)
    ...
    for start, end in zip(starts, ends):
        ...
        if start < end:
            for idx in range(start, end + 1):
                ...
                cur_pos, cur_sentiment = cur_ts_tag.split("-")
                assert cur_pos == "T"
                assert cur_sentiment == prev_sentiment, (
                    words,
                    ts_tag_sequence,
                    start,
                    end,
    ...
    for i in range(n_records):
        ...
        if term_texts != []:
            ...
            example = SemEvalExample(
                str(i), words, term_texts, starts, ends, new_polarities
            )
