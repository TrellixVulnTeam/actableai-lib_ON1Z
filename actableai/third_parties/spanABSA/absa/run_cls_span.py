...
from actableai.third_parties.spanABSA.absa.utils import (
    read_absa_data,
    convert_absa_data,
    convert_examples_to_features,
    RawFinalResult,
    wrapped_get_final_text,
    id_to_label,
)
...
def read_train_data(args, tokenizer, logger):
    ...
    train_examples = convert_absa_data(
        dataset=train_set, verbose_logging=args.verbose_logging
    )
    train_features = convert_examples_to_features(
        train_examples, tokenizer, args.max_seq_length, args.verbose_logging, logger
    )
    ...
    all_span_starts = torch.tensor(
        [f.start_indexes for f in train_features], dtype=torch.long
    )
    all_span_ends = torch.tensor(
        [f.end_indexes for f in train_features], dtype=torch.long
    )
    all_labels = torch.tensor(
        [f.polarity_labels for f in train_features], dtype=torch.long
    )
    all_label_masks = torch.tensor(
        [f.label_masks for f in train_features], dtype=torch.long
    )
...
def read_eval_data(args, tokenizer, logger):
    ...
    eval_examples = convert_absa_data(
        dataset=eval_set, verbose_logging=args.verbose_logging
    )
    eval_features = convert_examples_to_features(
        eval_examples, tokenizer, args.max_seq_length, args.verbose_logging, logger
    )
    ...
    all_span_starts = torch.tensor(
        [f.start_indexes for f in eval_features], dtype=torch.long
    )
    all_span_ends = torch.tensor(
        [f.end_indexes for f in eval_features], dtype=torch.long
    )
    all_label_masks = torch.tensor(
        [f.label_masks for f in eval_features], dtype=torch.long
    )
...
def pipeline_eval_data(args, tokenizer, logger):
    ...
    all_span_starts = torch.tensor(
        [f.start_indexes for f in eval_extract_preds], dtype=torch.long
    )
    all_span_ends = torch.tensor(
        [f.end_indexes for f in eval_extract_preds], dtype=torch.long
    )
    all_label_masks = torch.tensor(
        [f.span_masks for f in eval_extract_preds], dtype=torch.long
    )
    all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
    eval_data = TensorDataset(
        all_input_ids,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % save_checkpoints_steps == 0 and count != 0:
                logger.info(
                    "step: {}, loss: {:.4f}".format(global_step, running_loss / count)
                )

            if (
                global_step % save_checkpoints_steps == 0
                and global_step > start_save_steps
                and count != 0
            ):  # eval & save model
                ...
                f = open(log_path, "a")
                print(
                    "step: {}, loss: {:.4f}, P: {:.4f}, R: {:.4f}, F1: {:.4f} "
                    "(common: {}, retrieved: {}, relevant: {})".format(
                        global_step,
                        running_loss / count,
                        metrics["p"],
                        metrics["r"],
...
def metric_max_over_ground_truths(
    metric_fn, term, polarity, gold_terms, gold_polarities
):
    hit = 0
...
def eval_absa(
    all_examples, all_features, all_results, do_lower_case, verbose_logging, logger
):
    unique_id_to_result = {}
    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        pred_polarities = []
        print("---------------- hey ------------------")
        for start_index, end_index, cls_pred, span_mask in zip(
            result.start_indexes, result.end_indexes, result.cls_pred, result.span_masks
        ):
            if span_mask:
                final_text = wrapped_get_final_text(
                    example,
                    feature,
                    start_index,
                    end_index,
        ...
        for term, polarity in zip(pred_terms, pred_polarities):
            common += metric_max_over_ground_truths(
                exact_match_score,
                term,
                polarity,
                example.term_texts,
    ...
    p = common / retrieved if retrieved > 0 else 0.0
    r = common / relevant
...
def evaluate(
    ...
    metrics, all_nbest_json = eval_absa(
        eval_examples,
        eval_features,
        all_results,
        args.do_lower_case,
...
def main():
    ...
    parser.add_argument(
        "--data_dir", default="data/semeval_14", type=str, help="SemEval data dir"
    )
    parser.add_argument(
        "--train_file", default=None, type=str, help="SemEval xml for training"
    )
    parser.add_argument(
        "--predict_file", default=None, type=str, help="SemEval csv for prediction"
    )
    parser.add_argument(
        "--extraction_file", default=None, type=str, help="pkl file for extraction"
    )
    parser.add_argument(
        "--init_checkpoint",
        default=None,
    ...
    logger.info(
        "torch_version: {} device: {} n_gpu: {}, distributed training: {}, 16-bits training: {}".format(
    ...
    if args.init_checkpoint is not None and not os.path.isfile(save_path):
        ...
        logger.info(
            "Loading model from pretrained checkpoint: {}".format(args.init_checkpoint)
        )
    ...
    if os.path.isfile(save_path):
        ...
        logger.info(
            "Loading model from finetuned checkpoint: '{}' (step {})".format(
                save_path, step
            )
        )
    ...
    for n, param in model.named_parameters():
        print(
            "name: {}, size: {}, dtype: {}, requires_grad: {}".format(
    ...
    if args.do_train:
        ...
        eval_examples, eval_features, eval_dataloader = read_eval_data(
    ...
    if os.path.isfile(save_path):
        ...
        step = checkpoint["step"]
        logger.info(
            "Loading optimizer from finetuned checkpoint: '{}' (step {})".format(
                save_path, step
            )
        )
        global_step = step
    ...
    if args.do_predict:
        ...
        print(
            "step: {}, P: {:.4f}, R: {:.4f}, F1: {:.4f} (common: {}, retrieved: {}, relevant: {})".format(
                global_step,
                metrics["p"],
                metrics["r"],
                metrics["f1"],
    ...
    if args.do_pipeline:
        logger.info("***** Running prediction *****")
        eval_examples, eval_features, eval_dataloader = pipeline_eval_data(
            args, tokenizer, logger
        )
        ...
        f = open(log_path, "a")
        print(
            "pipeline, step: {}, P: {:.4f}, R: {:.4f}, F1: {:.4f} (common: {}, retrieved: {}, relevant: {})".format(
                global_step,
                metrics["p"],
                metrics["r"],
                metrics["f1"],
